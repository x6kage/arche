...
**Akademia**：4ロール構造と全ての機能はPaper 002で確立された通り維持される。唯一の変更は、**Scribe**から**Grapheus**への改称である（γραφεύςに由来、「書く者/記す者」 — 「graphics」「graph」「-ography」の語源的根）。これはロール名をギリシャ語命名規則に揃えつつ、機能を維持する — 論文の構造化、二言語出版、知識抽出。GrapheusはGrammateus（Layer 3）とは異なり、前者は研究を出版し、後者は運用上の計画と記録を管理する。
...
Strategosはアテネのstrategosに対応する — 戦略的決定を相当な戦術的自律性をもって執行する選出された作戦指揮官。VPoEの機能 — エンジニアリング実行の管理、レビューの実施、エンジニアリング基準の維持 — はまさにこれであり、戦略的方向性の下での作戦指揮である。

Epistatesはアテネのプリュタネイスの epistates に対応する — 統治評議会の日程の日常的調整を担当する議長である。PMOの機能 — プログラム間の調整、スケジュール管理、ワークストリームの整合 — は、このロールの直接的な類似物である。
...
**検討した代替案**：CDOを「Oikonomos」（οἰκονόμος、「家政官」）として維持すること。却下理由は、研究方向やガバナンス監督から独立した戦略的知識方向という独自の機能は、Layer 1のロールを正当化するほど実質的ではないこと。ポリスモデルにおいて、知識は専用の最高責任者を必要とする企業資産ではなく、制度的機構（研究のためのAkademia、キュレーションのためのMnemon、ガバナンスのためのCouncil Knowledge席）を通じて管理される共有の公共財である。
...
**検討した代替案**：CIOを「Epistates」（ἐπιστάτης、「監督者」）として維持すること。却下理由は二つ。(a) Epistatesの名はPMOにより適していた。(b) 技術的権限なきインフラ管理は、問題を特定できるが解決できないロールを作り出し、Polemarchへの恒常的なエスカレーションを必要とする。
...
**推論**：COO（最高運用責任者）の機能は運用効率の指標とワークフロー最適化であった。「プロセスの監督」（Thesmothete/Oversight）と「プロセスの最適化」（COO）の境界を検討した。区別はこうである。監督がプロセス違反を検知し、最適化がプロセス効率を改善する。実際には、コンプライアンスのためにプロセスを監視するエンティティは必然的に効率改善を特定する — 両機能に必要な情報は同一である。機能を分離すると、同じ領域を観察し同じ結論を争う二つのロールが生まれる。

**検討した代替案**：COOを「Logistes」（λογιστής、「会計検査官」）として維持すること。却下理由は、Logistesの歴史的機能 — 事後の財務/運用監査 — が、継続的な運用管理よりもCouncil Performance席（Seat 9）との重複が大きいこと。Councilは既に事後の効率評価を提供している。
...
**Skopos (Watcher)**：情報ギャップの検知は、Mnemonの知識維持と相補的な機能である。Mnemonは既知のものをキュレートし、Skoposは未知のものを特定する。一方を他方に吸収すると、知識基盤を維持しつつ同時に挑戦するロールが生まれてしまう — 内部的な利益相反である。
...
追加で必要な改訂（範囲は記載するが全文は実装に委ねる）は次の通り。
...
1. **意味的活性化の差異**：LLMがロール定義とともに「Archon」として行動するようプロンプトされた場合、その行動分布（一方的決定対内閣協議への傾向で測定）は、同等のロール定義で「CEO」としてプロンプトされた同一のLLMと測定可能に異なる。予測される方向は、「Archon」のフレーミングの下でより少ない一方的決定。20+セッションでの統制比較により検証可能。
...
| 日付 | 修正内容 |
|------|-----------|
| 2026-04-21 | グローバル認可状態ファイルの運用上の改名を反映するため、`governance-state.md` への 1 箇所の参照を `governance.md` に更新した。この改名は 2026-04-20 に Regulation 9 Emergency State Repair（Telos override、Article 11；当時は placeholder の呼称 "Founder" で呼ばれていた主権者）として実施されたもので、本稿のロールアーキテクチャに関する結論とは無関係である。理論的内容はいかなる形でも変更されていない — §7 移行サマリーにおける参照先ファイル名のみを更新する。Scholarch directive v0.0.4-PR-001-D1 による承認。 |
| 2026-06-14 | **校正パス（proofreading pass、Grapheus）。体裁のみ；内容・論旨・主張・反証可能な予測の意味は不変。** ja.md の誤字を2件修正：§4.4 の "epistatess" → "epistates"（綴り誤り、en は "epistates of the prytaneis"）、§5.5 の "Skop" → "Skopos"（語の欠落）。あわせて当該2文のてにをはを微調整。用語一貫性（Archontes / Strategoi / Hegemones / Technitai、各ギリシャ語ロール名）と bilingual 整合（表 / mermaid なし / §7 改正案コードブロックは英語維持 / 付録A / Corrections Log の 1:1 対応）を確認。en.md は誤字脱字なし（変更なし）。Telos 表記統一、Founder 残存・冠詞残り無し。 |
| 2026-06-14 | **；/： 日本語化パス（Grapheus、Telos 指示）。体裁のみ；内容・論旨・主張・反証可能な予測の意味は不変。** ja.md の散文・表中の全角 `；`/`：` を自然な日本語へ調整。**`；`：** ロール機能表（§4.3-§4.6, §3.1）のセル内リスト区切りを `、` へ、散文・参考文献（§5.2 結論、§8.1 確信度、参考文献の理論/階級/起源の併記）の `；` を `。`/`、` へ。**`：`：** §1-§9 の散文中の定義導入コロン（「考えよう：」「である：」「対応する：」等）を `。`/`——` 等の自然な構文へ。**保持：** 太字ラベルに続く見出し的コロン（`**推論**：`/`**結論**：`/`**確信度**：`/`**検討した代替案**：` 等 — en の `**Reasoning**:` 等と並行する構造ラベル）、標題コロン（§1 タイトル・付録A 見出し）、追記専用の歴史 Corrections-Log 行。en.md は変更なし。 |
