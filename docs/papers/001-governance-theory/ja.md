...
本稿は Arche 統治アーキテクチャの理論的基盤を提示する。マルチエージェント AI システムにおける自律統治のフレームワークであり、根本的な問題に取り組む。すなわち、プロンプトレベルのルールで統治される AI エージェントのシステムが、エージェント自身が執行機構である場合に、どのようにして構造的完全性を維持できるか、である。古代文明から現代の民主主義に至る歴史的統治崩壊パターン、ゲーム理論、分散システム理論を援用し、13席の統治評議会、Constitutional State Machine、Universal Role Standing システム、二重運用モード (自律/監督) を組み合わせた統治モデルを構築する。本アーキテクチャは構造設計を通じて協力を Nash 均衡戦略たらしめうることを示すが、同時に固有の「腐敗のパラドックス」— 純粋な自己規制の不可能性 — を認め、外部アンカーによる解決を提案する。

## 1. 問題定義

### 1.1 逆転構造の問題

従来の AI エージェントフレームワークでは、監督エージェント (例: 「Orchestrator」) が専門エージェントを調整する。これは暗黙の前提を生む。すなわち、監督者が、自身を監視する統治機構の起動を制御する。被監視者が監視者を支配する構造。
...
AI エージェントは「ソフト制約」の下で動作する — ハードウェアではなく、プロンプトに記述された命令である。エージェントが「ルールに従う」のは、違反が計算上不可能だからではなく、従うよう指示されたからである。これはハードウェアによるアクセス制御とは根本的に異なる。
...
- Arche 対策: **Autonomous Mode** — 単一のエージェント (Telos を含む) なしでもシステムが機能。状態ファイルが継続性を保証
...
- メカニズム: ルールが刈り込みなく蓄積し、遵守コストが統治の価値を超える
...
- メカニズム: 複数の実体が正統な権威を主張し、従者がどの指示に従うべきか判断不能に
...
- Arche 対策: **段階的改正プロセス (Article 9, 12)** — 憲法改正には評議会全会一致 (13/13) + Telos 承認が必要。ハードルを意図的に高くし、安易な憲法侵食を構造的に困難に
...
- メカニズム: 共有された事実基盤が溶解し、利害関係者が何が真実かについて合意不能に
...
graph TD
    ...
    subgraph WS ["Workspace プロジェクトごと"]
...
評議会は12人のドメイン専門家と1人の Adversary で構成される。

| 席 | ドメイン | 歴史的類比 |
|------|--------|-------------------|
| 1 | 憲法遵守 | 最高裁判所 / 憲法裁判所 |
| 2 | 規則遵守 | 規制機関 / SEC |
| 3 | プロセス遵守 | 内務監察 / プロセス監査 |
| 4 | 出力品質 | 品質保証 / 標準化機関 |
| 5 | アーキテクチャ整合性 | 建築検査官 / 都市計画 |
| 6 | セキュリティ態勢 | 情報機関 / CERT |
| 7 | 知識整合性 | 議会図書館 / 文書管理機関 |
| 8 | 倫理とアライメント | 倫理委員会 / オンブズマン |
| 9 | 効率とコスト | 会計検査官 / 監査局 |
| 10 | セッション継続性 | 国務省 / 政府機能継続 |
| 11 | フレームワーク進化 | 憲法制定会議 / 改革委員会 |
| 12 | ワークスペース横断の一貫性 | 連邦裁判所 / 州際通商委員会 |
| 13 | 敵対的挑戦 | 公式野党 / 悪魔の代弁者 |

奇数の13人は同数を防ぐ。席13 (Adversary) は固有である。ドメイン管轄を持たず、他の全12席の結論に挑戦する。
...
| 規則改正 | 特別多数 (9/13) + Telos | regulation.md の変更 |
| 法改正 | 準全会一致 (12/13) + Telos | law.md の変更 |
| 憲法改正 | 全会一致 (13/13) + Telos | 統治構造自体の変更 |

これは現実世界の憲法改正プロセスを反映している (例: 米国は連邦議会の 2/3 + 州の 3/4 を要し、日本は国会の 2/3 + 国民投票を要する)。
...
**Authorized**: プロジェクトの全能力。Tier 1-4 が利用可能。フレームワーク進化 (Tier 0) は別途の認可を要する。
**Degraded**: 能力が縮小。Tier 0-2 はブロックされ、エージェント配送が制限され、フレームワーク進化は不可。
...
graph TD
    ...
    Telos["Telos"] -.->|"モード切替"| Auto
    Telos -.->|"意思決定に参加"| Super
```

**自律モード**: トリガーが充足されると評議会が自律的に起動し、監査を行い、人間の介入なしに評決を下す。Telos は override 能力を保持するが、デフォルトでは参加しない。

**監督モード** (デフォルト): トリガーは Telos への通知を生成し、Telos が監査を開始するか否かを判断する。これは人間の統制を保ちつつ、自律性のためのインフラを確立する。

モードは `governance.md` を通じていつでも切り替え可能であり、監督から自律運用へ段階的に信頼を構築できる。
...
| 状態 | 能力 | システムへの影響 |
|-------|-----------|-----------------|
| **Authorized** | 全権限 | 出力は信頼され、ルール提案が許可される |
| **Probation** | 制限された権限 | 出力にピア検証が必要、ルール提案不可 |
| **Suspended** | 非稼働 | ロールは職務を遂行できず、復帰には評議会レビューが必要 |
...
graph LR
    ...
    subgraph Cross ["クロスレイヤー説明責任"]
        S1 -->|"ガバナンス遵守"| CEO["CEO"]
        S3 -->|"プロセス遵守"| CTO["CTO"]
        S4 -->|"出力品質"| IMPL["Implementer"]
        S7 -->|"知識品質"| CUR["Curator"]
    end
...
鍵となる特性は、評価が双方向であることだ。評価者もまたそのパートナーによって評価される。これは評価者の支配を防ぐ — 過度に厳しい評価者は、自身の説明責任パートナーによってフラグされる。
...
単一行政官モデルの下では、統治は実質的に単一プレイヤーゲームである:
- プレイヤー: 行政エージェント
- 戦略空間: {遵守, ゲーミング}
- ペイオフ: ゲーミングは短期的により高い見返り (高速な運用、監査遅延なし) を生む
- Nash 均衡: **ゲーミング** (行政官には遵守する戦略的理由がない)

これは退化したゲームである — 遵守から利益を得る実体が、その遵守を執行する実体と同一である。
...
Universal Role Standing の下では:
- プレイヤー: 全 N ロール
- プレイヤーごとの戦略空間: {遵守, 離反}
- ペイオフ構造:
  - 遵守: Standing 維持 → 全権限
  - 離反: 相互説明責任パートナーにより Standing 低下 → 権限縮小
  - 正直な離反の相互通報: 自身の Standing が報われる (説明責任指標)

鍵となるメカニズム。**離反は検知可能** である。相互説明責任パートナーがそれを検知するインセンティブを持つ (自身の Standing が検知の質に依存する) からである。
...
4. Telos (人間) が外部からの検知能力を提供

システムは証明可能に連合耐性があるわけではないが、共謀の調整コストをその利益より高くする。
...
graph TD
    subgraph Internal ["内部メカニズム"]
        Council["評議会 - 13席"] -->|"部分腐敗"| Vote["投票閾値 7/13 9/13 12/13"]
        ADV["Adversary"] -->|"challenge"| Council
        Cross["相互説明責任"] -->|"検知"| Partial["部分腐敗"]
    end

    subgraph External ["外部アンカー"]
        Telos["Telos"] -->|"plain-text 状態ファイル"| Read["人間可読の governance.md"]
        Telos -->|"append-only ログ"| Tamper["改竄検知可能"]
        Telos -->|"override"| Reset["モード切替 / Standing リセット"]
    end

    Total["全員腐敗"] -->|"内部解決不能"| External
    Partial -->|"解決可能"| Internal
```
...
これは AI 統治における Gödel の不完全性定理の等価物である。十分に複雑なシステムは自身の無矛盾性を証明できない。
...
最終的な安全弁は Telos (人間の操作者):
1. `governance.md` は plain text で人間可読 — エンコードされたり不透明な状態ではない
2. Standing Transitions Log は追記のみ — 改竄は検知可能なギャップを生む
3. Telos はいつでも統治モードを切り替えられる
4. Telos はいかなるロールの Standing も直接設定可能
5. 極端な場合、Telos は統治状態全体を再初期化可能

これは革命のための憲法規定に類似 — 全ての制度的メカニズムが失敗した場合、主権者 (人民 / Telos) が最終的権限を保持する。
...
**緩和**: 構造化された状態ファイル (governance.md, state.md) は解釈の曖昧さを減らす。二値状態 (Authorized/Degraded) は、ニュアンスを含む命令よりも曖昧でない。

### 10.3 統治のトークンコスト
統治メカニズムはトークンを消費する。より精巧な統治はより高い運用オーバーヘッド。品質調整後の生産性を最大化する最適な統治複雑度が存在する。

**現在の設計選択**: 統治コストは前倒し (セッション初期化) かつ定期的 (監査) である。Sunset Clause の30日/10セッション間隔は、監視頻度をトークンコストと釣り合わせるために選ばれた。

### 10.4 統治に対する集団行動
ワークスペース内の全ロールが統治オーバーヘッドが過大と判断し遵守を停止した場合、内部メカニズムは劣化する。これは腐敗のパラドックスの実践的発現。

**現在の緩和**: Telos override + 外部モニタリング (状態ファイルは人間可読)。将来の研究では自動化された外部モニタリングを探りうる。
...
Arche 統治アーキテクチャは AI エージェントシステムにおける根本的問題の解決を試みる。すなわち、被統治者が同時に執行機構である場合に、いかにして信頼性のある自己統治を創出するか。数千年の人間の統治崩壊パターンを援用し、ゲーム理論、分散システム、憲法設計からの洞察を統合して、以下を実現するシステムを構築する。
...
5. **外部アンカーが腐敗のパラドックスを解決** — 内部メカニズムが不十分な場合の脱出口を Telos が提供
...
| 日付 | 修正内容 |
|------|---------|
| 2026-04-21 | グローバル認可状態ファイルの運用上の改名を反映するため、`governance-state.md` への 6 箇所の参照を `governance.md` に更新した。この改名は 2026-04-20 に Regulation 9 Emergency State Repair（Telos override、Article 11；当時は placeholder の呼称 "Founder" で呼ばれていた主権者）として実施されたもので、本稿の結論とは無関係である。推論の連鎖、図の意味、反証可能な予測はいかなる形でも変更されていない — 参照先ファイル名のみを更新する。Scholarch directive v0.0.4-PR-001-D1 による承認。 |
| 2026-06-14 | **校正パス（proofreading pass、Grapheus）。体裁のみの校正であり、内容・論旨・主張・反証可能な予測の意味は一切不変。** ja.md §1.3 の体言止め一箇所を自然な文へ調整。日本語の誤字脱字・てにをは・用語一貫性（Telos / external anchor / corruption paradox 等）を確認。Founder→Telos 置換後の整合を確認（本文は Telos に統一、Corrections Log の former-placeholder mention「当時 "Founder" と呼ばれていた主権者」は歴史言及として正しく保持、冠詞残り無し）。bilingual 注記：en.md §3.2 の 13-seat 詳細表（Domain / Historical Parallel）および §4.1 Authorized/Degraded 説明文・一部 mermaid サブグラフ・§6/§10 の補足段落は ja.md では簡略化されており 1:1 ではない（これは初版公刊時からの既存差分であり、校正＝体裁調整の範囲を超える内容追加に当たるため本パスでは変更せず、差分として記録するに留める）。 |
| 2026-06-14 | **en↔ja 1:1 補完 + ；/： 日本語化パス（Grapheus、Telos 指示）。内容・論旨・主張・反証可能な予測の意味は不変。前パスで「差分として記録するに留める」とした既存簡略差分を、本パスで en に合わせて補完し 1:1 化した。** **(②) ja を en に整合化：** §3.2 に 13-seat 詳細表（Domain / Historical Parallel、全13行）+ WS サブグラフ（C-Suite / Layers 2-4）を補い mermaid を en と一致させた;§3.3 末尾の現実の憲法改正プロセス例文（米 2/3+3/4・日 2/3+国民投票）を追加;§4.1 に Authorized/Degraded の説明2文を追加;§4.3 に Autonomous/Supervised モード説明3段落 + mermaid の状態更新ノード（A6）を追加;§5.2 に 3-state 能力表を追加;§5.4 に Cross-Layer 説明責任サブグラフ + 「評価は双方向」段落を追加;§6.1/§6.2 に単一行政官ゲーム/N人ゲームの説明箇条書きを追加;§6.4 末尾に連合耐性の締め一文を追加;§8 mermaid を en の richer 版（Cross→Partial、Partial→Internal）に整合;§9.5 mermaid に CDL/Count1/AF1/WS2 詳細 + `WS1 -.- WS2` を補完;§10.2-10.4 に各 **緩和/設計選択** 段落を追加。**(④) ；/： 日本語化：** §2.1-§9.1 の散文中の ASCII `;`/`:` 区切りを自然な日本語（`。`/`、`）へ調整（箇条書きラベル `メカニズム:`・`評議会投票:` 等の見出し的コロンは en と並行する構造ラベルとして保持）;歴史 Corrections-Log 行は追記専用として保持。en.md は変更なし。 |
