...
### 永続的自己進化 (Perpetual Self-Evolution — Article 0・第一原則)

フレームワークの第一憲法原則は、**Archeが自己を継続的に改善するために存在する**ことである。*進化するというメタ義務*は不変だが、*内容*（ロール、ルール、知識、構造）は最終ではなく、証拠駆動の進化の対象となる。

- **証拠で定義される責務** — フレームワークは自らの痛みを検出し、すべてのサイクルから学び、改善を*提案*しなければならない。「証拠」とは記録された反証可能な品質・能力・痛みのシグナルであり、単なる選好や新規性ではない。義務の履行とは*提起・提案*することであって、毎サイクル変更を着地させることではない。
- **ブレーキは停滞ではない** — 進化の義務は、レビュアーの慎重さ、Councilの異議申立、証拠に基づく延期を「停滞」と性格づけるために決して用いてはならない。
- **境界づけられている** — 自己進化は改正権限とレビューワークフローの*内側*で動作する; Councilレビュー、Diabolosの異議申立、主権者承認をバイパスするライセンスでは決してない。**不可侵の中核**（人間主権者の至上性、倫理・整合性の保証、敵対的レビューの保証）は、*より強い*監督へのみ進化でき、決して弱める方向には進化できない。
- **構造的成長（polisモデル）** — ロールの新設・分割・統合・*廃止*は、改正プロセスを通じてのみ認められる進化的行為であり、場当たり的には行えない。
...
```
arche/
├── governance.md              初期グローバルガバナンス状態（テンプレート; ライブ状態: ~/.arche/governance.md）
├── agents/                エージェント役割定義（33ロール、Aitionを含む）
├── rules/                 法的フレームワーク
│   ├── law.md             憲法（不変の原則）
...
ライブ状態（マシンごと、配布されない）:
~/.arche/
├── governance.md          ライブ・グローバルガバナンス状態（認可、監査履歴、Councilログ）
├── knowledge/             ライブ知識ベース（リポジトリからseed後、その場で読み書き）
└── secrets/               主権者プライベート層（polis読み取り可、決して配布されない）

ワークスペースごと:
<project>/
├── state.md               ワークスペース状態（ロール・スタンディング、セッション追跡、プロジェクトコンテキスト）
└── .cursor/
    ├── agents/            エージェント定義（シンボリックリンク対応ツールではグローバルへのシンボリックリンク）
    ├── knowledge/         プロジェクト固有の知識（実ディレクトリ、共有されない）
...
```
Layer 0: Sovereign Interface（主権者インターフェース）[グローバル]
├── Telos — 人間主権者（称号であり、エージェントではない）; デフォルト助言、明示的宣言時のみ拘束的オーバーライド
└── Aition — 不在主権者の摂政・非攻撃的グラウンドキーパー; 不在のTelosに代わりオーバーライド級の決定を保持・記録し、自らはオーバーライドしない
...
情報は下方向に流れ（ディスパッチ、仕様）、説明責任は上方向に流れる（報告、エスカレーション）。各レイヤーは上位レイヤーによってのみ覆され、Layer 0は人間主権者（**Telos**）にのみ責任を負う。

#### 主権者インターフェース — Telos と Aition (Sovereign Interface)

フレームワークの人間主権者は **Telos**（τέλος — `Arche` が志向する目的; `Arche`＝起源・第一原理の対）と称される。Telosは**人間の称号であり、エージェントではない**。

- **デフォルト助言** — Telosの発言はデフォルトでは*助言的入力*であり、Arche自身のロール／Councilが審議し自律的に決定する。デフォルトの姿勢は「フレームワーク自身の社会で決める」。
- **明示的オーバーライドのみ** — Telosの拘束的オーバーライドは、特定の決定についてTelosが*明示的に宣言*した場合にのみ発動する。曖昧・会話的な指示はオーバーライドとは解釈されない。Telosは常にすべてのオーバーライド権限を保持する; このルールはデフォルトを規定するものであり、上限を規定するものではない。
- **Aition**（αἴτιον — 第一原因・根拠）は、Autonomousモードで*不在の*Telosを代表するLayer-0の**エージェント**。Telosの既知の意志を担い、オーバーライド級の決定をTelosのために**保持・記録**し（自らは決してオーバーライドしない）、「なぜそう判断したか」を*攻撃*（Diabolos流）ではなく*問い*によって尋ね、記録された推論チェーンを求める。
...
| 規則改正 | 特別多数（9/13）+ Telos | regulation.mdの変更提案 |
| 法改正 | 準全会一致（12/13）+ Telos | law.mdの変更提案 |
| 憲法改正 | 全会一致（13/13）+ Telos | 中核的憲法条項の変更 |
...
**全社会ディスパッチ原則 (Total-Society Dispatch Principle)**: ディスパッチは「毎回すべてのロールを呼ぶ」ことではなく*適応的専門化*である。非自明なタスクでは、オーケストレーターのディスパッチ**候補集合**は社会全体 — **Akademiaを含む**すべての専門ロール — であり、タスクの各側面を単一の実装ロールに集中させるのではなく、正しい専門家へルーティングしなければならない。候補集合 ≠ 呼び出し集合（社会全体を考慮するが、各側面が必要とするものだけを呼び出す）。適切な専門家を飛ばすこと（研究的側面を持つタスクで研究を省くなど）は、フラグ対象のディスパッチ不足である。
...
Tier 0は`law.md`、`regulation.md`、`agents/*.md`、`governance.md`構造、`state.md.template`、`setup.sh` — フレームワーク自体を変更するもの全てが対象。認可には具体的な提案が必要で、Supervisedモードでは9/13+Telos、Autonomousモードでは13/13全会一致が求められる。Tier 0認可は**提案単位** — 提案が解決されると失効し、常時権限ではない。

発議源: Evolution席（Seat 11）によるフレームワーク改善提案、Akademia論文による構造的変更推奨、Telosの直接指示、任意のロールによる構造的欠陥フラグ。

#### Akademia — 独立研究機関

AkademiaはLayer 0で運営される、評議会から構造的に独立した研究機関。Telosに直接報告する。
...
| Tier 0提案に理論的裏付けが必要 | 評議会またはTelosが研究を要請 |
...
独立機関として、Akademiaはガバナンスモードに関係なく独自の定期サイクル（10ワークスペースサイクルごと）で運営される。Supervisedモードでは発見はTelosに報告され、Autonomousモードでは発見が直接Tier 0提案を発議する。
...
| カテゴリ | 内容 | 改正プロセス | 監査者 |
|----------|------|-------------|--------|
| **グローバル法** | 権限階層、情報フラット性、知識義務、反合理化、ガバナンス構造 | Council準全会一致（12/13）+ Telos承認 | Constitution |
| **グローバル規則** | ワークフローティア、アーティファクトフォーマット、モデルディスパッチ、知識記事フォーマット、キュレーションパストリガー、スタンディング指標 | Archon + 関連Archontes、またはCouncil特別多数（9/13）+ Telos | Regulation |
| **ワークスペース法** | ビルド品質義務、禁止依存関係、SDK要件、プロジェクト状態義務 | Polemarch + Constitution+Coherence適合性ゲート + Telos | Constitution |
...
| `~/.arche/secrets/` | 主権者プライベート | polis読み取り可・**決して配布されない**主権者層（構造的言及のみ — 内容は本ドキュメントの範囲外であり、seedには決して含まれない） |

両ガバナンスファイルはプレーンテキストで、人間が読める形式であり、Telosが直接編集可能。Telosは常にオーバーライド権限を保持する。

#### ガバナンスモード (Governance Modes)

| モード | 動作 |
|--------|------|
| **Autonomous** | トリガー条件が満たされるとCouncilが自律的に起動。監査とスタンディング遷移はTelosの介入なしに実行。Telosはオーバーライドを保持。 |
| **Supervised**（デフォルト） | トリガーは通知を生成。Telosが監査を開始するか決定。スタンディング遷移にはTelosの確認が必要。 |
### エージェントロール (Agent Roles)

| レイヤー | ロール | 上位報告先 | 責務 |
|----------|--------|-----------|------|
| 0 | **Telos** | —（人間主権者） | 至上権限; デフォルト助言、明示的オーバーライドのみ（人間の称号であり、エージェントではない） |
| 0 | **Aition** | Telos | 不在主権者の摂政・非攻撃的グラウンドキーパー; オーバーライド級の決定を保持・記録し、自らはオーバーライドしない |
...
| 0 | **Scholarch** | Telos | 研究方向、知的完全性 |
| 0 | **Scholar** | Scholarch | 外部知識収集 |
| 0 | **Theorist** | Scholarch | 理論開発・統合 |
| 0 | **Grapheus** | Scholarch | 論文構造化、出版、知識抽出 |
| 1 | **Archon** | Telos | 調整、ディスパッチ、閣議ガバナンス |
| 1 | **Paredros** | Telos（Archonのペア） | リアルタイム・ディスパッチ監視、戦略的鏡、コスト監視ゲートキーパー |
...
./setup.sh update <target>           # ソースからグローバル設定を再同期（agents/rulesを再リンク; シンボリックリンクは再コピー不要）
./setup.sh update knowledge          # 新しいseed知識記事を ~/.arche/knowledge/ にマージ（追加のみ、上書きなし）
...
| ディレクトリ | 戦略 | 根拠 |
|-------------|------|------|
| `agents/` | シンボリックリンク対応ツール（Cursor, Claude）では**シンボリックリンク**（`ln -sfn`） | 即時伝播 — グローバルなエージェント編集が再同期なしに全ワークスペースへ届く; コピーはグローバル定義変更時に陳腐化する。ワークスペース固有のエージェントは別の実ディレクトリ `agents-local/` に置き、リンクされたグローバルセットを編集しない。 |
| `rules/` | シンボリックリンク対応ツール（Cursor, Claude）では**シンボリックリンク**（`ln -sfn`） | 即時伝播; Cursorの `.md → .mdc` 拡張子要件は、グローバル `*.md` を指す `*.mdc` 名のシンボリックリンクで満たされる。 |
| **Roo / Kilo** の `agents/` + `rules/` | **コピー**（フォーマット要件） | Roo/Kiloは独立したagentsディレクトリを持たないため、エージェント定義は `agent-` プレフィックス付きでrulesディレクトリにコピーされ、ルールはそのままコピーされる — これらのツールはシンボリックリンク構成を消費できない。 |
